...
DayR <- function(weather.data, min.year, max.year){
  ...
  for (iStation in (uStation)){
    ...
    for (iYear in (min.year:max.year)){
      tYear = subset(tStation, year == iYear)
      umonth  =  unique(tYear$month)  
      #Added 23 Jun 09 to remove missing values from US Data Set    
      for (i.month in (umonth)){
        t.month <- subset(tYear, month == i.month)
        rh.test <- sum(t.month$relativeHumidity == 999)
        t.test <- sum(t.month$temperature == 999.9)
        
        if(rh.test + t.test == 0){
          #End new addition 23 Jun 09
          if(i.month  ==  4 | i.month  ==  6 | i.month  ==  9 | i.month == 11){maxDay = 30} else
            if(i.month  ==  2){
              if(iYear/4 == round(iYear/4)){maxDay = 29} else {maxDay = 28}
            } else {maxDay = 31}
          for (iDay in (1:maxDay)){
            if (iYear < max.year | i.month < 12 | iDay < 31){
              ##Hours are 13:00 - 23:00, 0:00 - 12:00
              ##Note that the last day of the last year is excluded
              tDay <- subset(t.month, day == iDay)
